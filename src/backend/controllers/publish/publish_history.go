...
func (c *PublishController) List() {
	...
	if c.Ctx.Input.Query("type") != "" {
		param.Query["Type"] = c.Ctx.Input.Query("type")
	}
	if c.Ctx.Input.Query("resourceId") != "" {
		param.Query["ResourceId"] = c.Ctx.Input.Query("resourceId")
	}
