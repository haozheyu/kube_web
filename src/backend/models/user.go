...
func addDefaultNamespace(user *User) (err error) {
	...
	if err == orm.ErrNoRows {
		...
		demonNameSpaceID, err := beego.AppConfig.String("DemoNamespaceId")
		logs.Error("can not find default demo namespace by id "+demonNameSpaceID, err.Error())
		return nil
	} else if err != nil {
		return
	}
	...
	if err == orm.ErrNoRows {
		// 如果没有初始化角色，则不再初始化
		demongroupId, err := beego.AppConfig.String("DemoGroupId")
		logs.Error("can not find default demo group by id "+demongroupId, err.Error())
		return nil
	} else if err != nil {
		return
	}
