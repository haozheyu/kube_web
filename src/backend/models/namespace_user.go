...
func (*namespaceUserModel) Add(m *NamespaceUser, allGroupFlag bool) (id int64, err error) {
	...
	if allGroupFlag {
		for _, Group := range m.Groups {
			...
			_, _, err := Txo.ReadOrCreate(&newNamespaceUser, "Namespace", "User", "Group")
			if err != nil {
				continue
			}
		}
...
func (n *namespaceUserModel) UpdateById(m *NamespaceUser, allGroupFlag bool) (err error) {
	...
	if allGroupFlag {
		...
		_, err = Txo.QueryTable(TableNameNamespaceUser).Filter("Namespace", m.Namespace.Id).Filter("User", m.User.Id).RelatedSel("Group").All(&oldNUs)
		if err == nil {
			...
			for _, one := range diff {
				_, err = Txo.Delete(&NamespaceUser{
					Namespace: m.Namespace,
					User:      m.User,
