...
func (*groupModel) AddGroup(m *Group) (id int64, err error) {
	...
	if len(m.Permissions) > 0 {
		m2m := Txo.QueryM2M(m, "Permissions")
...
func (*groupModel) UpdateGroupById(m *Group) (err error) {
	...
	v := Group{Id: m.Id}
	if err = Txo.Read(&v); err != nil {
		return
	}
	m.UpdateTime = nil
	if _, err = Txo.Update(m); err != nil {
		Txo.Rollback()
		return
	}
	m2m := Txo.QueryM2M(m, "Permissions")
...
func (*groupModel) DeleteGroup(id int64) (err error) {
	...
	// 清除Permission的关联
	m2m := Txo.QueryM2M(&v, "Permissions")
	_, err = m2m.Clear()
	if err != nil {
		Txo.Rollback()
		return
	...
	if _, err = Txo.Delete(&Group{Id: id}); err != nil {
		Txo.Rollback()
		return
	}
	Txo.Commit()
